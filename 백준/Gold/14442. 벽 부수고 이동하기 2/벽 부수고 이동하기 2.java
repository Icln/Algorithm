		...
		while (!q.isEmpty()){
			...
			for (int i = 0; i < 4; i++) {
				...
				if (arr[nx][ny] == 0 && !visited[tmp.wall][nx][ny]){
					if (!visited[tmp.wall][nx][ny]){
						visited[tmp.wall][nx][ny] = true;
						q.offer(new Node(nx, ny, tmp.cnt + 1, tmp.wall));
					}
				}
				else if(tmp.wall < k && !visited[tmp.wall + 1][nx][ny]){
					visited[tmp.wall + 1][nx][ny] = true;
					q.offer(new Node(nx, ny, tmp.cnt + 1, tmp.wall + 1));
				}
			}
		}
		return -1;
